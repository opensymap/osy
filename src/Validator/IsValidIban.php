...
class IsValidIban extends Validator
{
    private $_iban_registry; = array();
    private  $this->_iban_mistranscriptions;
    ...
    public function check()
    {
        ...
        # Get country of IBAN
        $country = $this->iban_get_country_part($iban);

        # Test length of IBAN
        if(strlen($iban)!= $this->iban_country_get_iban_length($country)) {
            return 'Il codice Iban ha una lunghezza errata.';
        }

        # Get checksum of IBAN
        $checksum = $this->iban_get_checksum_part($iban);

        # Get country-specific IBAN format regex
        $regex = '/'.$this->iban_country_get_iban_format_regex($country).'/';

        # Check regex
        if(preg_match($regex,$iban)) {
            # Regex passed, check checksum
            if(!$this->iban_verify_checksum($iban)) {
                return 'Iban checksum errato.';
            }
        } else {
            return 'Iban errato.';
        }
    ...
    private function iban_to_machine_format($iban)
    {
    ...
    private function iban_to_human_format($iban)
    {
    ...
    private function iban_get_checksum_part($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
    ...
    private function iban_get_bban_part($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
    ...
    private function iban_find_checksum($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
    ...
    private function iban_set_checksum($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
    ...
    #  $s  Input string (IBAN)
    private function iban_mod97_10($s)
    {
    ...
    private function iban_get_parts($iban)
    {
        return array(
            'country'   =>  $this->iban_get_country_part($iban),
            'checksum'  =>  $this->iban_get_checksum_part($iban),
            'bban'      =>  $this->iban_get_bban_part($iban),
            'bank'      =>  $this->iban_get_bank_part($iban),
            'country'   =>  $this->iban_get_country_part($iban),
            'branch'    =>  $this->iban_get_branch_part($iban),
            'account'   =>  $this->iban_get_account_part($iban)
        );
    }

    # Get the Bank ID (institution code) from an IBAN
    private function iban_get_bank_part($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
        $country = $this->iban_get_country_part($iban);
        $start = $this->iban_country_get_bankid_start_offset($country);
        $stop = $this->iban_country_get_bankid_stop_offset($country);
        if($start!=''&&$stop!='') {
            $bban = $this->iban_get_bban_part($iban);
    ...
    private function iban_get_branch_part($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
        $country = $this->iban_get_country_part($iban);
        $start = $this->iban_country_get_branchid_start_offset($country);
        $stop = $this->iban_country_get_branchid_stop_offset($country);
        if($start!=''&&$stop!='') {
            $bban = $this->iban_get_bban_part($iban);
            return substr($bban,$start,($stop-$start+1));
        }
    ...
    private function iban_get_account_part($iban)
    {
        $iban = $this->iban_to_machine_format($iban);
        $country = $this->iban_get_country_part($iban);
        $start = $this->iban_country_get_branchid_stop_offset($country);
        if($start!='') {
            $bban = $this->iban_get_bban_part($iban);
    ...
    private function iban_country_get_country_name($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'country_name');
    }

    # Get the domestic example for an IBAN country
    private function iban_country_get_domestic_example($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'domestic_example');
    }

    # Get the BBAN example for an IBAN country
    private function iban_country_get_bban_example($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_example');
    }

    # Get the BBAN format (in SWIFT format) for an IBAN country
    private function iban_country_get_bban_format_swift($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_format_swift');
    }

    # Get the BBAN format (as a regular expression) for an IBAN country
    private function iban_country_get_bban_format_regex($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_format_regex');
    }

    # Get the BBAN length for an IBAN country
    private function iban_country_get_bban_length($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_length');
    }

    # Get the IBAN example for an IBAN country
    private function iban_country_get_iban_example($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'iban_example');
    }

    # Get the IBAN format (in SWIFT format) for an IBAN country
    private function iban_country_get_iban_format_swift($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'iban_format_swift');
    }

    # Get the IBAN format (as a regular expression) for an IBAN country
    private function iban_country_get_iban_format_regex($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'iban_format_regex');
    }

    # Get the IBAN length for an IBAN country
    private function iban_country_get_iban_length($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'iban_length');
    }

    # Get the BBAN Bank ID start offset for an IBAN country
    private function iban_country_get_bankid_start_offset($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_bankid_start_offset');
    }

    # Get the BBAN Bank ID stop offset for an IBAN country
    private function iban_country_get_bankid_stop_offset($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_bankid_stop_offset');
    }

    # Get the BBAN Branch ID start offset for an IBAN country
    private function iban_country_get_branchid_start_offset($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_branchid_start_offset');
    }

    # Get the BBAN Branch ID stop offset for an IBAN country
    private function iban_country_get_branchid_stop_offset($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'bban_branchid_stop_offset');
    }

    # Get the registry edition for an IBAN country
    private function iban_country_get_registry_edition($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'registry_edition');
    }

    # Is the IBAN country a SEPA member?
    private function iban_country_is_sepa($iban_country)
    {
        return $this->_iban_country_get_info($iban_country,'country_sepa');
    }

    # Get the list of all IBAN countries
    private function iban_countries()
    {
        global $this->_iban_registry;;
        return array_keys($this->_iban_registry;);
    }
    ...
    # mistranscriptions.
    private function iban_mistranscription_suggestions($incorrect_iban)
    {
        ...
        if(!$this->_iban_load_mistranscriptions()) {
            return array('(failed to load)');
        }

        # init
        global $this->_iban_mistranscriptions;
        $suggestions = array();
        ...
        for($i=0;$i<$length;$i++) {
            ...
            foreach($this->_iban_mistranscriptions[$character] as $possible_origin) {
                # if we're:
        ...
        foreach($char_freqs as $char=>$freq) {
            ...
            if($freq>1) {
                ...
                foreach($this->_iban_mistranscriptions[$char] as $possible_origin) {
    ...
    private function _iban_load_registry()
    {
        ...
        {
            $data = file_get_contents(dirname(__FILE__) . '/IsValidIbanData.txt');
            $lines = explode("\n",$data);
    ...
    private function _iban_get_info($iban,$code)
    {
    ...
    private function _iban_country_get_info($country,$code)
    {
        global $this->_iban_registry;;
        ...
        if(array_key_exists($country,$this->_iban_registry;)) {
            if(array_key_exists($code,$this->_iban_registry;[$country])) {
                return $this->_iban_registry;[$country][strtolower($code)];
            }
        }
        return false;
    }
    ...
    private function _iban_load_mistranscriptions()
    {
        # do not reload if already present
        if(is_array($this->_iban_mistranscriptions) && count($this->_iban_mistranscriptions) == 36) {
            return true;
        }
        $this->_iban_mistranscriptions = array();
        ...
        foreach($lines as $line) {
            ...
            if(preg_match('/^ *c-(\w) = (.*?)$/',$line,$matches)) {
                ...
                $this->_iban_mistranscriptions[$character] = $chars;
            }
        }
        return true;
    }
