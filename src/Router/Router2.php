...
class Router
{
    ...
    * @var array
    */
    protected $routesOriginal = array();
    ...
    */
    public function __construct($url = null)
    {
        if ($url == null) {
            ...
            $url = !empty($_SERVER['REQUEST_URL']) ? $_SERVER['REQUEST_URL'] : $_SERVER['REQUEST_URI'];
        }
        // Store the dirty version of the URL
        $this->urlDirty = $url;
        // Clean the URL, removing the protocol, domain, and base directory if there is one
        $this->urlClean = $this->getCleanUrl($this->urlDirty);
    }
   ...
   */
    public function run()
    {
        ...
        foreach ($this->routes as $priority => $routes) {
            ...
            if (preg_match($route, $this->urlClean, $matches)) {
                ...
                return array('callback' => $callback, 'params' => $params, 'route' => $route, 'original_route' => $this->routesOriginal[$priority][$route]);
            }
        }
    }

    // Was a match found or should we execute the default callback?
    if (!$matched_route && $this->defaultRoute !== null) {
        return array('params' => $this->urlClean, 'callback' => $this->defaultRoute, 'route' => false, 'original_route' => false);
    }
  }
    ...
    */
    public function route($route, $callback, $priority = 10)
    {
        ...
        $this->routesOriginal[$priority][$route] = $original_route;
        return true;
    }
